...
<body>
	...
	<%
		...
		}else out.print("<p>num1은 num2보다 작다.<p/>");
		...
		}else out.print("<p>num3이 가장 크다.<p/>");
	...
	<c:choose>
		<c:when test="${num1 gt num2 }"><p>num1은 num2보다 크다.</p></c:when>
		<c:when test="${num2 gt num3 }"><p>num2은 num3보다 크다.</p></c:when>
		<c:otherwise><p>num3가장 크다.</p></c:otherwise>
	</c:choose>
	
	<!-- 반복문 -->
	
	<c:forEach var="i" begin="1" end="5"><p>i : ${i}</p></c:forEach>
	
	<!-- 배열 -->
	
	<c:set var="people">김유신,김춘추,장보고,강감찬,이순신</c:set>
	<c:forEach var="person" items="${people}"><span>${person }</span><br/></c:forEach>
